...
import PutOptionABI from 'abi/put-option'
...
const PutStepper = ({ amount, onClose }) => {
  let { currentStep, failed } = usePutStepper(amount)
...
export default PutStepper

function usePutStepper (amount) {
  const web3 = useWeb3()
  ...
  useEffect(() => {
    ...
    const optionContract = new web3.eth.Contract(PutOptionABI, put)
